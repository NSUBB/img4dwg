...
def apply_color_overlay(image, color=(255, 0, 255), threshold=0, contrast=1.0):
    img = image.convert("L")
    ...
    color_array = np.zeros((inverted_array.shape[0], inverted_array.shape[1], 3), dtype=np.uint8)
    for i in range(3):
        color_array[:, :, i] = (normalized * color[i]).astype(np.uint8)

    result_img = Image.fromarray(color_array)
    return result_img
...
def process_images(folder_path, color, threshold, contrast, progress_callback, done_callback):
    ...
    for i, filename in enumerate(image_files):
        ...
        try:
            ...
            result_img = apply_color_overlay(image, color, threshold, contrast)
...
def load_preview():
    ...
    try:
        ...
        rgb = root.winfo_rgb(color)
        rgb = tuple(int(c / 256) for c in rgb)
        result_img = apply_color_overlay(image, rgb, threshold, contrast)
